...
namespace uPLibrary.Networking.M2Mqtt
{
    ...
    public class MqttClient
    {
        ...
        private async void InitAsync(string brokerHostName, int brokerPort, bool secure, X509Certificate caCert, string[] topic, Action<string[]> callback)
        {
            // throw exceptions to the caller
            var hostEntry = await Dns.GetHostEntryAsync(brokerHostName);
        ...
        // 비동기 방식으로 Mqtt 클라이언트 초기화 하는 생성자.
        public MqttClient(string brokerHostName, int brokerPort, bool secure, X509Certificate caCert, string[] topic, Action<string[]> callback)
        {
            InitAsync(brokerHostName, brokerPort, secure, caCert, topic, callback);
        }
