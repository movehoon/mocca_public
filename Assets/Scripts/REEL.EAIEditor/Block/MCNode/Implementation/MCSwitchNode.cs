...
{
    public class MCSwitchNode : MCNode
    {
        ...
        [SerializeField] private RectTransform switchElems;
        ...
        private void SetLastSockets(Vector2 yPos)
        {
            ...
            for (int ix = 0; ix < lastSockets.Length; ix++)
            {
                ...
                //Vector2 pos = lastSockets[i].anchoredPosition;
                //if (!isInputConnected)
                //{
                //    if (i == 0)
                ...
                //lastSockets[i].anchoredPosition = pos;
            }
        }
        ...
        private void SetSwitchElem()
        {
            SetInputFields();
            SetNexts();
        }

        public void RemoveSwitchElem(int index)
        {
        ...
        public void AddSwitchElem()
        {
            GameObject inputValue = Instantiate(inputPrefab, switchElems); //inputfield 생성
            inputFieldPrefabs.Add(inputValue.GetComponent<TMP_InputField>()); //inputfield List에 추가

            GameObject nextSocket = Instantiate(nextSocketPrefab, switchElems); //nextsocket 생성
            nextSocketPrefabs.Add(nextSocket.GetComponent<MCNodeNext>()); //nextSockets List에 추가

            ChangedSwitchElem();
        }

        private void ChangedSwitchElem()
        {
            SetSwitchElem(); //inputfields, nexts 초기화
            SetBlockSizeAndSocketPos(InputFieldPrefabCount); //블록사이즈, 첫번째 소켓 위치 설정
            SetSwitchElemsPos(); //MatchElem 위치 설정
            SetLastSockets(epLeft.anchoredPosition); //마지막 줄 위치 설정
        }

        private void SetSwitchElemsPos()
        {
            SetInputFieldsPos();
            SetNextSocketsPos();
        }
        ...
        private void SetNextSocketsPos()
        {
            ...
            {
                Vector2 Pos = new Vector2(-epLeft.localPosition.x, epLeft.anchoredPosition.y - i * ySizeOffset);
        ...
        public override void SetData(Node node)
        {
            ...
            for (int i = 0; i < inputFieldPrefabs.Count; i++)
            {
                inputFieldPrefabs[i].GetComponent<TMP_InputField>().text = node.nexts[i].value;
            }
