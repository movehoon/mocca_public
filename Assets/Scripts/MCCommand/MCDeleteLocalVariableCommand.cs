...
{
    public class MCDeleteLocalVariableCommand : MCCommand
    {
        ...
        public MCDeleteLocalVariableCommand(LeftMenuVariableItem variable)
        {
            ...
            variableIndex = MCWorkspaceManager.Instance.GetLocalVariableIndexWithName(name);
            if (variableIndex == -1)
            {
        ...
        public void Execute()
        {
            ...
            //Utils.LogRed($"deletedNodeInfos.Count : {deletedNodeInfos.Count }");
            ...
            // 변수 제거.
            variableFunctionManager.RemoveLocalVariable(MCWorkspaceManager.Instance.GetLocalVariable(variableIndex));
        }

        public void Undo()
        {
            // 변수 복원.
            MCWorkspaceManager.Instance.AddLocalVariable(name, type, nodeType, value, functionName, true);
            ...
            // 노드 복원.
            MCCommandHelper.RestoreDeletedNodes(deletedNodeInfos, true);
        ...
        {
            //Utils.LogRed($"[AddToDeletedNodeInfos] node.name: {node.nodeData.name}/node.body.isLocalVariable: {node.nodeData.body.isLocalVariable}");
